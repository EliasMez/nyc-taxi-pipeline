...
def test_setup_data_warehouse():
    ...
    """
    mock_cursor = Mock()
    with patch('snowflake_ingestion.init_infra_snowflake.run_sql_file') as mock_run_sql:
        with patch('snowflake_ingestion.init_infra_snowflake.logger') as mock_logger:
            setup_data_warehouse(mock_cursor)
            
            mock_run_sql.assert_called_once_with(mock_cursor, SQL_DIR / "setup_data_warehouse.sql")
...
def test_create_roles_and_user():
    ...
    with patch('snowflake_ingestion.init_infra_snowflake.run_sql_file') as mock_run_sql:
        with patch('snowflake_ingestion.init_infra_snowflake.logger') as mock_logger:
            create_roles_and_user(mock_cursor)
            
            mock_run_sql.assert_called_once_with(mock_cursor, SQL_DIR / "create_roles_and_user.sql")
...
def test_grant_privileges():
    ...
    with patch('snowflake_ingestion.init_infra_snowflake.run_sql_file') as mock_run_sql:
        with patch('snowflake_ingestion.init_infra_snowflake.logger') as mock_logger:
            grant_privileges(mock_cursor)
            
            mock_run_sql.assert_called_once_with(mock_cursor, SQL_DIR / "grant_privileges.sql")
...
def test_main_success():
    ...
    with patch('snowflake_ingestion.init_infra_snowflake.connect_with_role', return_value=mock_conn) as mock_connect:
            ...
            with patch('snowflake_ingestion.init_infra_snowflake.create_roles_and_user') as mock_create:
                with patch('snowflake_ingestion.init_infra_snowflake.grant_privileges') as mock_grant:
                    with patch('snowflake_ingestion.init_infra_snowflake.logger') as mock_logger:
                        ...
                        mock_connect.assert_any_call(USER, PASSWORD, ACCOUNT, 'SYSADMIN')
                        mock_connect.assert_any_call(USER, PASSWORD, ACCOUNT, 'SECURITYADMIN')
...
def test_main_exception():
    ...
    """
    with patch('snowflake_ingestion.init_infra_snowflake.connect_with_role', side_effect=Exception("Connection failed")):
        with patch('snowflake_ingestion.init_infra_snowflake.logger') as mock_logger:
            main()
