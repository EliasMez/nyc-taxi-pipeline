...
from trips
left join {{ ref('dim_date') }} as dd_pickup on trips.pickup_date = dd_pickup.date
left join {{ ref('dim_time') }} as dt_pickup on trips.pickup_time = dt_pickup.time
left join {{ ref('dim_date') }} as dd_dropoff on trips.dropoff_date = dd_dropoff.date
left join {{ ref('dim_time') }} as dt_dropoff on trips.dropoff_time = dt_dropoff.time
